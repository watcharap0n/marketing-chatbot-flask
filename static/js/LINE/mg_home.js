new Vue({
    ...
    vuetify: new Vuetify(),
    data: {
        products: ['โครงการบ้านแสนสุข', 'โครงการแมงโก้คอนโด', 'โครงการบ้านใหม่ทาวน์', 'โครงการแมงโก้โฮม'],
        others: ['Facebook', 'งานบ้านและคอนโด', 'WEBSITE โตรงการ', 'นิตยสารที่เกี่ยวข้องกับที่อยู่อาศัย'],
    ...
    delimiters: ["[[", "]]"],
    created() {
        liff.init({liffId: '1655208213-dx9Kj5kB'}, () => {
                if (liff.isLoggedIn()) {
                    liff.getProfile()
                        .then((profile) => {
    ...
    methods: {
        onSubmit() {
            ...
            if (validate === true) {
                this.spinBtn = false
                const path = '/api/all/questionnaire?collection=test_import'
